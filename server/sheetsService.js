...
function parseSheetHeaders(rows) {
  ...
  if (!bestMatch || bestMatch.score < 4) {
    throw new Error(
      "Could not detect player header and subheader rows in the Log sheet."
    );
  }
  ...
  if (parsedPlayers.length === 0) {
    throw new Error(
      "No players with both Duration (hh:mm) and Activity columns were found."
    );
  }
...
export class SheetsService {
  ...
  async getHeaderInfo(forceRefresh = false) {
    ...
    const range = `${this.sheetName}!A1:ZZ10`;
    ...
    const parsed = parseSheetHeaders(rows);
  ...
  async assertColumnMappingIsSafe(playerInfo, headerInfo) {
    const neededColumns = [
      playerInfo.columns.duration,
      playerInfo.columns.activity,
      playerInfo.columns.score
    ].filter((value) => value !== undefined);

    if (!neededColumns.length) {
      throw new Error("Could not validate player columns.");
    }
    ...
    const scoreHeader =
      playerInfo.columns.score !== undefined ? normalize(subheaderRow[playerInfo.columns.score]) : "score";

    if (durationHeader !== "duration (hh:mm)" || activityHeader !== "activity" || scoreHeader !== "score") {
      throw new Error(
        "Sheet columns did not pass safety validation. No data was written. Confirm the Log sheet headers are unchanged."
      );
    }
  }
  ...
  async findSuggestedDate({ dateRows, normalizedDate, durationColumn, activityColumn, scoreColumn }) {
    ...
    for (const rowInfo of rowsToCheck) {
      const rowState = await this.getRowState(
        { durationColumn, activityColumn, scoreColumn },
        rowInfo.row
      );
  ...
  async upsertPlayerEntry({ player, date, duration, activity, addToSameDay = false }) {
    ...
    if (!targetRowInfo) {
      throw new Error(
        "Selected date was not found in existing sheet rows. Choose a date that already exists in the Log sheet."
      );
    }
    ...
    const scoreColumn =
      playerInfo.columns.score !== undefined
        ? columnIndexToLetter(playerInfo.columns.score)
        : null;
    ...
    if (rowState.hasExistingDuration || rowState.hasExistingActivity) {
      if (addToSameDay) {
        ...
        if (!existingDuration || !existingActivity || existingActivity === "-") {
          throw new Error(
            "Cannot combine this row because existing duration/activity data is incomplete."
          );
        }
        ...
        const shouldAddMixedWorkoutNote =
          scoreColumn &&
          normalize(existingActivity) !== normalize(activity);
        ...
        await this.sheets.spreadsheets.values.batchUpdate({
          ...
          requestBody: {
            ...
            data: [
              {
                range: `${this.sheetName}!${durationColumn}${targetRow}`,
                values: [[mergedDuration]]
              },
              {
                range: `${this.sheetName}!${activityColumn}${targetRow}`,
                values: [[mergedActivity]]
              }
            ]
          }
        });
        ...
        const mergedScore = scoreColumn
          ? await this.getScoreValue(scoreColumn, targetRow, existingScoreFormulaText)
          : null;
      ...
      const baseMessage =
        `This row already has data (Duration: ${rowState.existingDuration || "(blank)"}, Activity: ${
          rowState.existingActivity || "(blank)"
        }).`;

      if (suggestionResult?.date) {
        const occupiedError = buildDateOccupiedError(
          `${baseMessage} Suggested date: ${suggestionResult.date}.`,
          suggestionResult
        );
      ...
      if (suggestionResult?.reachedCheckLimit) {
        const occupiedError = buildDateOccupiedError(
          "All previous rows appear taken. Please select another date.",
          null
        );
        occupiedError.mergePreview = mergePreview;
        throw occupiedError;
