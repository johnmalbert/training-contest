...
class SheetsService {
  ...
  async getRecentPlayerWorkouts(player, limit = 3) {
    ...
    const scoreColumnIndex = playerInfo.columns.score !== undefined ? playerInfo.columns.score : null;

    const minColumnIndex = Math.min(durationColumnIndex, activityColumnIndex, scoreColumnIndex ?? activityColumnIndex);
    const maxColumnIndex = Math.max(durationColumnIndex, activityColumnIndex, scoreColumnIndex ?? activityColumnIndex);
